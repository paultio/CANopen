...
uint8_t CANopen::read8bit(\
  ...
  composeMsg(SDO_REQUEST_READ,index,subIndex);
  mbed::CANMessage msg(id+SDO_COMMAND_ID_BASE, can_msg_buffer, 8);

#ifdef DEBUG
  Serial.print("Sending SDO Read 8bit to ID: 0x");
  Serial.println(id+SDO_COMMAND_ID_BASE, HEX);
  Serial.print("Data: ");
  for (int i = 0; i < 8; ++i)
  {
    Serial.print("0x");
    if (can_msg_buffer[i] < 0x10)
      Serial.print('0');
    Serial.print(can_msg_buffer[i], HEX);
    Serial.print(' ');
  }
  Serial.println();
#endif
...
uint8_t CANopen::read32bit(\
  ...
  mbed::CANMessage msg(id+SDO_COMMAND_ID_BASE, can_msg_buffer, 8);
#ifdef DEBUG
  Serial.print("Sending SDO Read 32bit to ID: 0x");
  Serial.println(id+SDO_COMMAND_ID_BASE, HEX);
  Serial.print("Data: ");
  for (int i = 0; i < 8; ++i)
  {
  ...
  Serial.println();
#endif
  if (!sendCanBuffer(id+SDO_COMMAND_ID_BASE,8)) {
#ifdef DEBUG
    Serial.println("Failed to send SDO Read 32bit message");
#endif
    return FAILURE;
  }
#ifdef DEBUG
  Serial.println("SDO Read 32bit message sent, waiting for response...");
#endif
...
uint8_t CANopen::sendCanBuffer(\
  ...
  mbed::CANMessage msg(id, can_msg_buffer, length);
...
uint8_t CANopen::composeMsg(\
  ...
  // for(int i=4; i<8; i++) {
  //   can_msg_buffer[i] = 0x00; // clear data bytes
  // }
  return SUCCESS;
}
...
uint8_t CANopen::receiveCanMsg() {
  ...
  // Copy data from CANMessage to receive buffer
  for (uint8_t i = 0; i < length && i < 8; i++) {
    can_receive_buffer[i] = msg.data[i];
  }
